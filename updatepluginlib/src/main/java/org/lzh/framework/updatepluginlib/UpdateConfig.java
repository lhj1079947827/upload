...
public final class UpdateConfig {
    ...
    /**
     * 注意当改参数为false时候方法  setUpdateParser中response为空,返回的Update由用户定义下载地址、版本号、名字、等。
     * 当netWork=false时 setUrl不在起作用
     * @param netWork 用于是否执行网络请求获取app下载信息
     * @return
     */
    public UpdateConfig setNetWork(Boolean netWork){
        if (this.entity==null){
            this.entity = new CheckEntity().setNetWork(netWork);
        }else{
            this.entity.setNetWork(netWork);
        }
        return this;
    }
    ...
    public CheckEntity getCheckEntity () {
        if (this.entity==null){
            throw new IllegalArgumentException("CheckEntity is empty");
        }

        if (this.entity.getNetWork()&&TextUtils.isEmpty(this.entity.getUrl())){
            throw new IllegalArgumentException("CheckEntity url can not be empty");
        }
